...
In `scripts` folder there is script `run_rfdetr_video.py` which run RF-DERT object detection model on video frames and saves results to the `data/detections.json` file, so we can cache detection results.
